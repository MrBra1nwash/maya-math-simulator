...
function generateDistractor(correctAnswer: number, existing: Set<number>, allowNegative: boolean): number {
  const strategies = [
    () => correctAnswer + 1,
    () => correctAnswer - 1,
    () => correctAnswer + 2,
  ...
  for (const strategy of shuffledStrategies) {
    ...
    if (!existing.has(value) && value !== correctAnswer && (allowNegative || value >= 0)) {
      return value
    }
  }
...
export function generateChoices(correctAnswer: number, difficulty: DifficultyLevel, allowNegative = false): number[] {
  const count = getChoiceCount(difficulty)
  const used = new Set<number>([correctAnswer])
  const choices = [correctAnswer]

  while (choices.length < count) {
    const distractor = generateDistractor(correctAnswer, used, allowNegative)
    used.add(distractor)
